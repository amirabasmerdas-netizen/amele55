...
async def _clock_loop(cl, owner_id):
    """به‌روزرسانی ساعت نام و بیو با دقت بالا - دقیقاً در لحظه تغییر دقیقه"""
    last_updated = ""
    
    while True:
        try:
            # ✅ محاسبه دقیق زمان باقی‌مانده تا شروع دقیقه بعدی
            now = datetime.datetime.now()
            seconds_until_next_minute = 60 - now.second
            
            # اگر دقیقاً در لحظه تغییر دقیقه هستیم، 60 ثانیه صبر کن
            if seconds_until_next_minute == 0:
                seconds_until_next_minute = 60
            
            # منتظر می‌مونیم تا دقیقه بعدی شروع بشه
            await asyncio.sleep(seconds_until_next_minute)
            
            # ✅ الان دقیقاً لحظه تغییر دقیقه است
            time_str = persian_time()
            
            # فقط اگه زمان تغییر کرده، به‌روزرسانی کن
            if time_str != last_updated:
                font_id = db.get_setting(owner_id, "selected_font", "0")
                fn = FONTS.get(font_id, FONTS["0"])
                styled_time = fn(time_str)
                
                # به‌روزرسانی هم‌زمان نام و بیو
                updates = []
                if db.get_setting(owner_id, "clock_name_active") == "1":
                    updates.append(UpdateProfileRequest(last_name=styled_time[:64]))
                if db.get_setting(owner_id, "clock_bio_active") == "1":
                    updates.append(UpdateProfileRequest(about=f"⏰ {styled_time}"[:70]))
            ...
            # در صورت خطا، ۱ ثانیه صبر کن و ادامه بده
