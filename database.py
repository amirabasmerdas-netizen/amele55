...
def deduct_tokens(owner_id: int, amount: int) -> bool:
    try:
        ...
        if not result or result['balance'] < amount:
            return False
        query = "UPDATE amel_tokens SET balance = balance - %s WHERE owner_id = %s AND balance >= %s"
        execute_query(query, (amount, owner_id, amount))
        return True
    except Exception:
        return False
...
# ─── بقیه توابع ──────────────────────────────────────────────────────────────
# (رفرال، دشمن، دوست، پیام‌ها - مشابه قبل با کوئری‌های PostgreSQL)
...
def get_pending_scheduled(owner_id: int):
    try:
        query = """
            ...
            ORDER BY send_at
        """
        result = execute_query(query, (owner_id, datetime.datetime.now().isoformat()), fetch_all=True)
        return [dict(r) for r in result] if result else []
    except Exception:
        return []
